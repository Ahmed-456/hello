...
extern uint8_t audio_blaze;


//extern volatile uint8_t Mode;
volatile bool     charge_lock = false;
volatile bool     ble_a121_status = false;
...
void A121_hit_send_ble(float x,float y,float z,uint16_t distance)
{
  length = 3;
  uint8_t op_snd[4] = {pod_color,0x01,base_code};
  uint8_t json_dat[100];
  sprintf(json_dat, "{\"X\":%.2f, \"Y\":%.2f, \"Z\":%.2f, \"D\":%d}",x,y,z,distance);
  printf("%s\n",json_dat);
  printf("[");
  for (uint8_t i = 0; i < length; i++)
  {   
...
/*void A121_tap_send_ble(float x,float y,float z,uint16_t distance)
{
  length = 3;
  uint8_t op_snd[4] = {pod_color,0x01,base_code};
  uint8_t json_dat[100];
...
void A121_wave_send_ble(uint16_t distance)
{
  length = 3;
  uint8_t op_snd[4] = {pod_color,0x01,base_code};
  uint8_t json_dat[20];
  sprintf(json_dat, " {\"D\": %d} ",distance);
  printf("%s\n",json_dat);
  printf("[");
  for (uint8_t i = 0; i < length; i++)
  {   
...
void A121_tap_fail_send_ble(void)
{
  length = 3;
...
void A121_cyc_count_send_ble(int count)
{
  char str[5];
  length=15;
...
void A121_trigger_send_ble(void)
{
  length=16;
