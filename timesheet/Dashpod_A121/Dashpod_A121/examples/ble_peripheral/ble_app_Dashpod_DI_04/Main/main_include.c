...
//extern volatile bool     ble_A121_status;
...
bool app_sensor_all_setting_set(uint8_t *p_in, uint16_t in_len)
{
        ...
        bool status=true;
        uint8_t A121_accel_both = p_in[0];
        #ifdef DEBUG
        ...
        LED_LS(true);
        if(A121_accel_both == 0x01)
        {
          setup_A121_sensitivity(p_in[1]);
          printf("Wave function\n");
          Mode = 3;
          //tap_fun = false;
          //ble_A121_status = true;
        }
        else if (A121_accel_both == 0x02)
        {
...
void Batt_level_meas_timeout_handler(void * p_context)
{
  ...
  //A121_cyc_count_send_ble(read_batt_per());
  #ifdef DEBUG
...
void int_fuel_pin_enable(void)
{
  if(!chrg_in_en)
  {
    nrf_drv_gpiote_in_event_enable(IO_IN, true);
    chrg_in_en = true;
    chrg_in_dis = false;
  }
}
...
void int_fuel_pin_disable(void)
{
  if(!chrg_in_dis)
  {
    nrf_drv_gpiote_in_event_disable(IO_IN);
    chrg_in_en = false;
    chrg_in_dis = true;
  }
}
...
void gpio_init(void)
{
    ...
    err_code = nrf_drv_gpiote_in_init(IO_IN, &in_config, in_fuel_pin_handler);
    APP_ERROR_CHECK(err_code);
    int_fuel_pin_enable();
    //nrf_drv_gpiote_in_event_enable(IO_IN, true);
}
...
void TW_Dashpod_init(void)
{
    ...
    nrf_delay_ms(100);
    bq2505_set_current_limit_mode(7);//935mA
    nrf_delay_ms(50);
    ...
    setup_fuel_guage(&m_twi,0x36,1000);
   #ifndef DEBUG
   ...
   printf("\nDashpod initalization completed\n\n");
    #ifdef DEBUG
    printf("Checking for A121 radar sensor\n");
    #endif
    //Mode = 3;
...
void TW_dashpod_start_loop(void)
{
  ...
  while(Mode)
  {
    switch(Mode)//Mode
    {
      case 1:
        int_fuel_pin_disable();
        nrf_drv_gpiote_in_uninit(IO_IN);
        tap_detect_exit();
        gpio_init();
        //Charge_timer_start();
        Mode = 4;
...
void TW_dashpod_wait_loop(void)
{
    for(;wait_loop;)
    {
        hal_test_spi_read_chipid();        //for testing the A121 module responding or not for given input
    }
