...
#define POWER_SAVE_MODE   (ACC_CONFIG_IDLE_STATE_READY)
float prev=0;
...
float RANGE_START_M  = 0.07f;

float RANGE_LENGTH_M = 1.2f;

uint8_t UPDATE_RATE_HZ  = 48U;

uint8_t SWEEPS_PER_FRAME  = 16U;

uint8_t HWAAS  = 60U;

uint8_t PROFILE = ACC_CONFIG_PROFILE_2;

uint8_t audio_blaze = 0;
...
void setup_A121_sensitivity(uint8_t sens)
{
      #ifdef DEBUG
      printf("in A121 setup\n");
      #endif
       switch(sens)
       {
              case 0: 
                      RANGE_START_M  = 0.05f;
                      RANGE_LENGTH_M = 0.20f;
                      UPDATE_RATE_HZ  = 60U;
                      SWEEPS_PER_FRAME = 16U;
                      HWAAS = 32U;
                ...
                if(!seq || !cool_trig || !cool_time)
                {
                        ...
                        if (status)
                        {
                              ...
                              if (!cool_time && cool_trig)
                              {
                               ...
                               if (wave_to_exit)
                               {
                                  dist = result.presence_distance*100;
                                  printf("\nWave detected\n");
                                  ...
                                  i = 110;
                                    ...
                                    { 
                                         ...
                                         { 
                                                ...
                                                A121_hit_send_ble(data[0],data[1],data[2],dist);                                
                                                seq = true;
                                                //pscore = 0;
                                                LED_LS(false);
                                                //stop_beep();
          ...
          if(!seq)
          {
              ...
              short int data_main[3];
                  ...
                  { 
                      read_accel(&m_twi,0x6b,(uint8_t *)data_main);
                      process_raw_accel(data_main, data);
                      A121_hit_send_ble(data[0],data[1],data[2],0);
                      seq = true;
                      LED_LS(false);
                      #ifdef DI_04
                      if(pod_action != 1 && pod_action != 2)
